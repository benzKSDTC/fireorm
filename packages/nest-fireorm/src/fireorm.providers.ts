import { EntitySchema, CollectionRepository, CollectionGroupRepository } from '@benzksd/fireorm'
    ...
    if (options.collectionGroups) {
        collectionGroupProvide = options.collectionGroups.map(collection => ({
            ...
            useFactory: (firestore: Firestore) =>
                CollectionGroupRepository.getRepository(collection.entity, firestore, collection.collectionId),
            inject: [FIRESTORE_INSTANCT],
        }))
    }
